...
(function() {
  ...
  FormbuilderCollection = (function(_super) {
    ...
    FormbuilderCollection.prototype.findWhereParentUuid = function(uuid) {
      return this.where({
        'parent_uuid': uuid
      });
    };
  ...
  ViewFieldView = (function(_super) {
    ...
    ViewFieldView.prototype.duplicate = function(e) {
      ...
      if (attrs.options.grid) {
        var childModels = this.model.collection.findWhereParentUuid(attrs.parent_uuid);
        var parentModel = this.model.collection.findWhereUuid(attrs.parent_uuid);
        var maxRows = parseInt(parentModel.attributes.options.num_rows);

        if (childModels.length == maxRows) {
          delete attrs['parent_uuid'];
          delete attrs['options']['grid'];
          this.parentView.createField(attrs, {
            position: this.model.collection.length + 1
          });
        } else {
